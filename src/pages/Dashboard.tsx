  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-steel">نظرة عامة على أداء أنسام</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={formatMoney(t.income_total)}
        />
        <StatCard
          icon={TrendingDown}
          tone="red"
          label="إجمالي المصروفات"
          value={formatMoney(t.expense_total)}
        />
        <StatCard
          icon={FileText}
          tone="navy"
          label="قيمة الفواتير"
          value={formatMoney(t.invoices_total)}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div className="card flex items-center gap-3 p-4">
          ...
          <div>
            <div className="text-xl font-bold text-navy">{t.employees_count || 0}</div>
            <div className="text-xs text-steel">موظف نشط</div>
          </div>
        </div>
        <Link to="/app/invoices/new" className="card flex items-center gap-3 bg-navy p-4 text-white hover:bg-navy-700">
          <FileText className="h-8 w-8" />
          <div>
            <div className="text-sm font-bold">فاتورة جديدة</div>
            <div className="flex items-center gap-1 text-xs text-sky-soft/80">
              إصدار مباشر <ArrowUpLeft className="w-3 h-3" />
            </div>
          </div>
        </Link>
      </div>
